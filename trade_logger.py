        ...
        if trades:
            ...
            for trade in trades:
                if trade[3] == coin_symbol:  # from_currency
                    # the / 5 is meant to make the price more stable and not fluctuate too much based on a single trade
                    total_price += (trade[1] / trade[2]) * trade[7] / 5  # boska/other * amount)
                else:  # to_currency
                    total_price += (trade[2] / trade[1]) * trade[8] / 5  # other/boska * amount
